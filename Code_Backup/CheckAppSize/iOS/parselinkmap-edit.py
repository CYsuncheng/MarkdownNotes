        ...
        for base_bundle_map in base_bundle_list:
            ...
            if base_name == target_name:
                ...
                if base_size_value < target_size_value:
                    print "%s%s%s" % (target_name.ljust(50), str("%.3fM" % base_size_value).ljust(10),
                                      str("%.3fM" % target_size_value).ljust(10))
                    pass
                break
            pass
        if has_bundle_in_base == 0:
            print "%s%s%s%s" % (target_name.ljust(50), str("%.3fM" % base_size_value).ljust(10),
                                str("%.3fM" % target_size_value).ljust(10), "Y".center(10))
            pass
        pass

def compare_reduce(base_bundle_list, target_bundle_list):
    print "%s" % "=".ljust(80, '=')
    print "%s" % "模块大小减少列表".center(84)
    print "%s" % "=".ljust(80, '=')
    print "%s%s%s%s" % ("模块名称".ljust(54), "基线大小".ljust(14), "目标大小".ljust(14), "是否移除模块".ljust(14))
    for target_bundle_map in target_bundle_list:
        target_name = target_bundle_map["name"]
        target_size = target_bundle_map["size"]
        if target_size == "0.00M":
            continue
        target_size_value = float(target_size.split("M")[0])
        has_bundle_in_base = 0
        base_size_value = 0
        for base_bundle_map in base_bundle_list:
            ...
            if base_name == target_name:
                ...
                has_bundle_in_base = 1
                if base_size_value < target_size_value:
                    print "%s%s%s" % (target_name.ljust(50), str("%.3fM" % target_size_value).ljust(10),
                                      str("%.3fM" % base_size_value).ljust(10))
                    pass
                break
            pass
        if has_bundle_in_base == 0:
            print "%s%s%s%s" % (target_name.ljust(50), str("%.3fM" % target_size_value).ljust(10),
                                str("%.3fM" % base_size_value).ljust(10), "Y".center(10))
            pass
        pass
...
def main():
    ...
    if need_compare == 1:
        ...
        print "\n\n\n"
        compare_reduce(target_bundle_list, base_bundle_list)
